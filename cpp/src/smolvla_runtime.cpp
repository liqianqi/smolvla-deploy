...
SmolVLARuntime::SmolVLARuntime(const RuntimeConfig& cfg)
  ...
  // ---- 配置执行后端:优先 TensorRT EP(FP16),否则 CUDA EP ----
  if (cfg_.use_tensorrt) {
    OrtTensorRTProviderOptions trt{};
    trt.device_id = 0;
    trt.trt_fp16_enable = cfg_.fp16 ? 1 : 0;
    trt.trt_engine_cache_enable = cfg_.trt_cache_dir.empty() ? 0 : 1;
    trt.trt_engine_cache_path = cfg_.trt_cache_dir.c_str();
  ...
  auto path = [&](const char* f) { return cfg_.artifacts_dir + "/" + f; };
  vision_ = std::make_unique<Ort::Session>(env_, path("vision_encoder.onnx").c_str(), so);
  assembler_ = std::make_unique<Ort::Session>(env_, path("prefix_assembler.onnx").c_str(), so);
  prefill_ = std::make_unique<Ort::Session>(env_, path("vlm_prefill.onnx").c_str(), so);
  denoise_ = std::make_unique<Ort::Session>(env_, path("action_denoise_step.onnx").c_str(), so);

  // 加载动作反归一化统计量: [mean(D), std(D)] 的 float32, D=real_action_dim
  if (cfg_.unnormalize_action) {
    std::string stats = cfg_.action_stats_path.empty() ? path("action_norm_stats.bin")
...
// ---- Block 2: prefix 组装 ----
void SmolVLARuntime::RunAssembler(const std::vector<Ort::Value>& image_embeds, const Observation& obs) {
  ...
  auto outs = assembler_->Run(Ort::RunOptions{nullptr}, in_names, ins.data(), ins.size(), out_names, 4);
  ...
  prefix_pad_.assign(reinterpret_cast<const uint8_t*>(pad), reinterpret_cast<const uint8_t*>(pad) + L);
...
void SmolVLARuntime::RunPrefill() {
  ...
  auto outs = prefill_->Run(Ort::RunOptions{nullptr}, in_names, ins.data(), ins.size(), out_names, 2);

  const size_t kv_n =
      static_cast<size_t>(d_.num_layers) * L * d_.kv_heads * d_.head_dim;  // B=1
...
std::vector<float> SmolVLARuntime::RunDenoiseStep(const std::vector<float>& x_t, float t) {
  ...
  auto outs = denoise_->Run(Ort::RunOptions{nullptr}, in_names, ins.data(), ins.size(), out_names, 1);
