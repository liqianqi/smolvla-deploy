...
struct RuntimeConfig
{
    ...
    bool use_tensorrt = true;   // true: TensorRT EP; false: CUDA EP
    bool fp16 = false;          // 全局 FP16(实测会打崩本模型输出, 仅实验用)
    // 按引擎开 FP16(诊断/混合精度用), 逗号分隔: "vision,assembler,prefill,denoise"
    // 非空时覆盖全局 fp16 开关; 各引擎的 TRT 缓存自动按精度分目录
